...
TEST_EXE_PATH := $(BUILD_DIR)/CarterTest
ifeq ($(WITH_WINDOWS),1)
...
icon: prepare
	convert -background white -fill black \
	  -size 128x64 -gravity center label:".:* Carter *:." "$(PROJECT_DIR)/docs/icon.png"

grind:
	mkdir -p log
	$(GRIND) $(GRIND_OPTS) "$(BUILD_DIR)/./CarterTest" > log/debug 2> log/error